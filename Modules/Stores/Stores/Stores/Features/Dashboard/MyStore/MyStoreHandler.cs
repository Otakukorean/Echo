...
public record MyStoreResponse(StoreDto Store);

public class MyStoreHandler(StoresDbContext dbContext) : IQueryHandler<MyStoreQuery, MyStoreResponse>
{
    public async Task<MyStoreResponse> Handle(MyStoreQuery query, CancellationToken cancellationToken)
    {
        ...
        return new MyStoreResponse(new StoreDto(
